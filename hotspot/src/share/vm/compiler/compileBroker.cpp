...
void CompileBroker::compiler_thread_loop() {
  ...
  while (!is_compilation_disabled_forever()) {
    ...
    /*if (UseAOSDBOptCompile && mongo_index != -1)
    {
      ...
      std::string sig = std::string(task->method()->signature ()->as_C_string ());
      std::string fullDesc = name + sig;
      if (UseAOSDBVerbose)
        std::cout<<"getting optLevel for " + fullDesc << std::endl;
      
      int optLevel = mongo_aosdb_getOptLevelForMethod (mongo_index, fullDesc);
      if (UseAOSDBVerbose)
        std::cout<<"optLevel for " + fullDesc << " is " << optLevel << " current level is " << task->comp_level()<< std::endl;
      if (optLevel > 0 && optLevel <= 3 && optLevel > task->comp_level())
        task->set_comp_level (optLevel);
    }*/
