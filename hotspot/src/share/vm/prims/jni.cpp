...
#ifndef USDT2
HS_DTRACE_PROBE_DECL1(hotspot_jni, mongodb_start__entry, vm);
DT_RETURN_MARK_DECL(mongodb_start, jint);
#else
DT_RETURN_MARK_DECL(mongodb_start, jint
                    , HOTSPOT_JNI_mongodb_start_RETURN(_ret_ref));
#endif

_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_mongodb_start (JavaVM **vm)
{
  #ifndef USDT2
  HS_DTRACE_PROBE1(hotspot_jni, mongodb_start__entry, vm);
#else 
  HOTSPOT_JNI_mongodb_start_ENTRY(
                                 (void **) vm);
#endif 
/*
  jint result = JNI_ERR;
  DT_RETURN_MARK(mongodb_start, jint, (const jint&)result);

  std::cout<< "mongodb_start" << std::endl;
  if (UseAOSDBVerbose)
     ;//mongo_aosdb_setVerbose ();
  
  ClassPathEntry* e = ClassLoader::classpath_entry(0);
  while (e != NULL)
  {
    std::cout<< "Entry: " << e->name () << std::endl;
    e = e->next();
  }
  Handle loader (Thread::current(), SystemDictionary::java_system_loader());
  
  {
     ClassLoaderData* cld = ClassLoaderDataGraph::_head;
        int j = 0;
        while (cld != NULL && cld->_class_loader != NULL)
        {
          //try
          if (cld->_class_loader == NULL)
            std::cout << "CLASS LOADER NULL" << std::endl;
          if (cld->_class_loader->klass() == NULL)
            std::cout << " cld->_class_loader->klass() is NULL " << std::endl;
          if (cld->_class_loader->klass()->name() == NULL)
            std::cout << "cld->_class_loader->klass()->name () is NULL"<< std::endl;
          std::cout<< "CLASS LOADER " << cld->_class_loader->klass()->name()->as_C_string () << std::endl;
          cld = cld->next();
          j++;
  ...
  {
    mongo_aosdb_initialize ();
    mongo_aosdb_readAllDocuments ();
    int not_inits = 0;

    for (int i = 0; i < mongo_aosdb_getNMethods (); i++)
    {
      MongoMethodDatabaseElement* elem = mongo_aosdb_getMethod (i);
      string className = elem->getClassName ();
      string methodName = elem->getMethodName ();
      std::cout<<elem->getMethodFullDesc () << std::endl;
      std::cout<< className << "   " << methodName << std::endl;
      if (className.find ("jikesrvm") != string::npos)
          continue;
      
      TempNewSymbol sym = TempNewSymbol (SymbolTable::new_symbol(className.c_str (), Thread::current()));
      if (sym == NULL)
      {
        std::cout<<"SYM IS NULL "<< className << std::endl;
        continue;
      }
      
      Klass* ik = SystemDictionary::resolve_or_null(sym, Handle(), Handle(), Thread::current());
      instanceKlassHandle k (Thread::current(), ik);
      if (k.not_null()) {
        k->initialize(Thread::current());
        std::cout<< "INITIALIZED " << className <<std::endl;
      }
      else
      {
        std::cout<< "NOT INITIALIZED by null handler " << className <<std::endl;
        ClassLoaderData* cld = ClassLoaderDataGraph::_head;
        int j = 0;
        while (cld != NULL)
        {
          //try
            ...
            Handle loader (Thread::current(), cld->_class_loader);
            ik = SystemDictionary::resolve_or_null(sym, loader, Handle(), Thread::current());
            k = instanceKlassHandle(Thread::current(), ik);
            if (k.not_null()) {
              k->initialize(Thread::current());
              std::cout<< "INITIALIZED " << className << " by loader " << j << std::endl;
              break;
            }
          else
          //catch (...)
          {
            std::cout<< "Cannot find " << className << " by loader " << j << std::endl;
          }
        ...
        {
          not_inits++;
          std::cout<<"NOT INITIALIZED AT ALL "<<not_inits << std::endl;
        }
      ...
      */
      /*Method *method = NULL;
      //k->find_method(method_name, method_signature);
      for (int n = 0; n < k->methods()->length(); n++)
      {
        char *method_cstring = k->methods()->at(n)->name()->as_C_string();
        std::cout<<"Class method "<< method_cstring << " searching for " << methodName << endl;
        if (strcmp (method_cstring, methodName.c_str ()) == 0)
        {
            method = k->methods()->at(n);
        ...
        */
              ...
              if (can_be_compiled(m, comp_level)) {
                ...
                if (HAS_PENDING_EXCEPTION) {
                  ...
                  tty->print_cr("CompileTheWorld (%d) : Skipping method: %s", _compile_the_world_class_counter, m->n  ame_and_sig_as_C_string());
                } else {
